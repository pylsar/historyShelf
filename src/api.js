...
axiosApiInstance.interceptors.response.use((response) => {
    ...
    const authStore = useAuthStore();
    const originalRequest = error.config; //запоминаем конфиг
    ...
    if(error.response.status == 401 && !originalRequest._retry){
        ...
        try{
            ...
            console.log(err);
            //если ошибка то разлогиневаемся
...
// axios.interceptors это перехватчики, которые срабатывают при каждом запросе. для удобной работы с token, чтобы добавить 1 раз а не каждый раз